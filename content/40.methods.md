...
To enhance the accessibility of BioChatter, we have introduced a WebAssembly (WASM) deployment of Language Models (LLMs), leveraging the web-llm library available at https://github.com/mlc-ai/web-llm. This deployment is accessible through webGPU-compatible browsers, including Chrome version >113, Edge >113, Opera >99, and Firefox nightly builds. The WASM binary, compiled using the web-llm library, is integrated into the server docker image, making it seamlessly available to the BioChatter server within ChatGSE-Next.This innovative approach not only ensures a secure conversational environment but also mitigates the risks associated with potential data breaches. For instance, the ChatGSE-Next server can be hosted within a local network or even on a local computer. Utilizing the hosted LLM model requires no internet connection or powerful server; instead, computational power is drawn directly from the user's browser. This deployment method guarantees complete data security and is only constrained by the resources of the user's computer. While achieving a similar outcome is possible with a local deployment of the Xorbits Inference API, the browser-based deployment stands out for its user-friendly interface and superior scalability. This approach enhances the overall user experience, making BioChatter more accessible and efficient for a broader audience.
